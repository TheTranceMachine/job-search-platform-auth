...
// Enable CORS
// const whitelist = ['http://localhost:5173', 'https://us-south.appid.cloud.ibm.com', 'http://localhost:8080'];
// app.use(
// 	cors({
...
passport.use(new WebAppStrategy({
	...
	redirectUri: `http://localhost:5173` + CALLBACK_URL
}));
...
app.post(ROP_SUBMIT, function (req, res, next) {
	passport.authenticate(WebAppStrategy.STRATEGY_NAME, function (err, user, info, options = { keepSessionInfo: true }) {
		...
		if (!user) {
			return res.status(404).json({ message: info.code })
		}
